...
export function TopNav({ onExport, onKnowledge }: { onOpenAi?: () => void; onExport: () => void; onKnowledge: () => void }) {
  ...
  return (
    <header className="h-14 sm:h-16 bg-surface border-b border-border flex items-center justify-between gap-2 px-3 sm:px-8 shrink-0 z-20 sticky top-0 print:hidden mt-[10px]">
      ...
      <div className="flex items-center gap-1 sm:gap-4 overflow-x-auto">
        ...
        <Button variant="ghost" size="sm" onClick={() => fileInputRef.current?.click()} className="h-9 px-2 sm:px-3">
          <Upload className="w-4 h-4" /><span className="hidden sm:inline">导入</span>
        </Button>
        <Button variant="ghost" size="sm" onClick={handleExportJson} className="h-9 px-2 sm:px-3">
          <Download className="w-4 h-4" /><span className="hidden sm:inline">JSON</span>
        </Button>
        <Button variant="ghost" size="sm" onClick={resetProject} className="h-9 px-2 sm:px-3">
          <RotateCcw className="w-4 h-4" /><span className="hidden sm:inline">新建</span>
        </Button>
        <Button variant="ghost" size="sm" onClick={onKnowledge} className="hidden sm:inline-flex h-9 px-3">
          <BookOpen className="w-4 h-4" /> 知识库
        </Button>
        <Button variant="ghost" size="sm" onClick={onExport} className="hidden sm:inline-flex h-9 px-4">
